...
**Do NOT try to write directly to persistent storage:**

```gcode
❌ WRONG - Silent failure:
#2070=1175(Enter probe speed...)  // Fails! #1175 is persistent storage

✅ CORRECT - Two-step pattern:
#2070=105(Enter probe speed...)   // Step 1: Input to temp variable
#1175=#105                        // Step 2: Copy to persistent
```

**Why**: Firmware limitation causes silent write failures outside #50-#499 range. Variable ends up with garbage/default values instead of user input.

**See**: `input-dialog-patterns.md` for complete safe usage patterns
...
**⚠️ CRITICAL: Variable Range HARD LIMIT [CONFIRMED]**

**#2070 can ONLY write to #50-#499 - attempting to write to persistent storage causes SILENT FAILURES**

**❌ WRONG - Direct write to persistent**:
```gcode
#2070=1175(Enter probe speed...)  ; ❌ Silent failure!
; User types 400, but #1175 ends up with 1 or garbage value
```

**✅ CORRECT - Two-step pattern (ALWAYS use this)**:
```gcode
...
**Affected persistent ranges** (cannot be used directly with #2070):
- #1153-#1193 (user persistent storage)
- #2039-#2071 (user persistent storage)
- #2500-#2599 (user persistent storage)
- #500-#999 (parameter mirrors - don't use anyway)

**See**: `input-dialog-patterns.md` for complete two-step pattern examples
...
- User must TYPE values - pressing Enter alone gives wrong results
