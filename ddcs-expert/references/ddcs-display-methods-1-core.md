...
> **⚠️ Evidence note (2026-05-07):** Example below uses `=` (not `==`) and `GOTO 1` with space — both violate the skill's own C-style and no-space rules. Production macros never use `IF #var = N` (single equals). Recommended: rewrite as `IF #1505==0 GOTO1`. Whether the single-`=` form is actually accepted is untested.

```gcode
#1505 = 3(Move Right[Enter] or Left[Esc]?)
IF #1505 = 0 GOTO 1   ; ⚠ violates own rules — see note above
G01 X10 F200          ; ENTER pressed
GOTO 2                ; ⚠ space before label
N1
