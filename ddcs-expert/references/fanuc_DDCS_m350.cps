...
properties = {
 ...
 // --- CUSTOM EDIT START: Safe Park & Hidden Options ---
  safePositionMethod: {
    title      : "Safe Retracts",
    ...
    type       : "enum",
    values     : [
      {title:"G53 (Safe Z Only)", id:"G53"},
      {title:"G28 (Reference Point)", id:"G28"}
    ],
    value: "G53", // Default to Safe Z Only
    scope: "post" // Show in post dialog for user selection
  },
  safeZHeight: {
    title      : "Safe Z Height (machine)",
    description: "Machine coordinate Z height for safe retracts (e.g. -1).",
    group      : "homePositions",
    type       : "number",
    value      : -1,
    scope      : "post"
  ...
  homePositionEnd: {
    ...
    type       : "enum",
    values     : [
      {title:"Safe Park (Teach with K6)", id:"safePark"},
      {title:"Tool Change (Teach with K7)", id:"tool"},
      {title:"Machine Home (X0 Y0)", id:"home"},
      {title:"Safe Z Only", id:"safeZ"}
    ],
    value: "safePark", 
    scope: "post"
  },
  ...
  useCoolant: {
    ...
    description: "Forces M8 (Pump On) when spindle starts. Uncheck for dry runs/testing.",
    group      : "preferences",
    type       : "boolean",
    value      : true,
    scope      : "post"
...
var settings = {
  ...
  maximumSequenceNumber   : 99999999, // the maximum sequence number (Nxxx), use 'undefined' for unlimited
...
function onOpen() {
  ...
  writeln("%");
  // Removed O-word output for DDCS compatibility
  ...
  if (getSetting("headPositioningMethod", 0) == 1 && machineConfiguration.isHeadConfiguration() && !is3D() && tcp.isSupportedByMachine && getSetting("workPlaneMethod.useTiltedWorkplane", false)) {
    for (var i = 0; i < getNumberOfSections(); ++i) {
      ...
      if (!section.isMultiAxis() && defineWorkPlane(section, false).isNonZero()) {
        writeln("");
        var msg = "Warning, NC program requires parameter 5006 Bit6 to be set to 1." + EOL + "Ensure that the G49 command does not cause axis movement on your machine.";
        warning(msg);
        writeBlock("IF[PRM[5006,6]NE1]THEN#3000=91", formatComment("Parameter_5006_Bit_6_must_be_set_to_1"));
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    ...
    // 1. Start Spindle (M3/M4)
    writeBlock(sOutput.format(spindleSpeed), mFormat.format(tool.clockwise ? 3 : 4));
    
    // 2. Force Water Pump (M8) if switch is ON
  ...
  case COMMAND_LOAD_TOOL:
    if (getProperty("useM6")) {
      ...
      writeComment("T" + toolFormat.format(tool.number)); // Just write (T1) as a comment
    }
    writeComment(tool.comment);

    var preloadTool = getNextTool(tool.number != getFirstTool().number);
    if (getProperty("preloadTool") && preloadTool && getProperty("useM6")) {
...
function onClose() {
  ...
  // 1. Vertical Safety Retract
  writeBlock("#150 = " + xyzFormat.format(getProperty("safeZHeight")));
  writeBlock(gFormat.format(53), "Z#150"); // DDCS safe: no G0/G00

  // >>> THE FIX: Tell the post-processor "Yes, we are officially retracted!" <<<
  ...
  // 2. Horizontal Park Move (Based on Dropdown)
  var endPos = getProperty("homePositionEnd");

  if (endPos == "safePark") {
    // Moves to 'Safe Park' (Taught by K6) -> Output: G53 G0 X[#601] Y[#602]
    writeBlock(gFormat.format(53), "X[#601]", "Y[#602]");
    
  } else if (endPos == "tool") {
    // Moves to 'Tool Change' (Taught by K7) -> Output: G53 G0 X[#603] Y[#604]
    writeBlock(gFormat.format(53), "X[#603]", "Y[#604]");
    
  } else if (endPos == "home") {
    // Machine Home (X0 Y0)
    writeBlock("#151 = 0");
    writeBlock("#152 = 0");
    writeBlock(gFormat.format(53), "X#151", "Y#152");
  }
  // -------------------------------
...
/**
  ...
  (Modified to FORCE REMOVE Sequence Numbers)
*/
function writeBlock() {
  ...
  // --- HARD CODED: Never output N-numbers ---
...
function writeComment(text) {
  ...
  // --- STRICT FILTER: Remove Personal Use Warnings ---
  var _checkText = String(text).toUpperCase();
  
  // Block any line containing these specific phrases
  if (_checkText.indexOf("PERSONAL USE") > -1 || 
      _checkText.indexOf("FUSION SUBSCRIPTION") > -1 || 
      _checkText.indexOf("RAPID MOVES") > -1 ||
      _checkText.indexOf("MOVES IS REDUCED") > -1 || 
      _checkText.indexOf("AVAILABLE WITH A FUSION") > -1) {
    return; // Stop! Do not print this comment.
  }
  // ----------------------------------------------------
...
function writeToolBlock() {
  // Just write the tool line without messing with properties
...
function getRetractParameters() {
  ...
  var method = getProperty("safePositionMethod", "G53");
  ...
  if (isFullCircle()) {
    ...
    default:
      if (getProperty("allow3DArcs")) {
        error(localize("3D arcs (G02.4/G03.4) are not supported on DDCS Expert M350. Disable 'Allow 3D Arcs' in post properties."));
        return;
      } else {
        linearize(tolerance);
      }
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      case "G30":
        ...
        break;
      case "G53":
        // Only output G53 Z#150 for safe Z retract (no G0/G00 for DDCS safety)
...
function getOffsetCode(forceTCP) {
  if (!getProperty("useG43")) {
    state.lengthCompensationActive = true; // FIX: Fake it so Fusion doesn't error
    return ""; // Return nothing (No G43 output)
  }
