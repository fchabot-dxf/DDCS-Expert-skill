...
> **⚠️ Style-consistency note (2026-05-07):** Many examples in this file use spaces before GOTO labels (e.g. `IF #1921 == 0 GOTO 1` — see lines ~80-115, 162, 360, 389, 446) and spaces around `==`. The skill's other docs (`CORE_TRUTH.md`, `conditional-syntax-card.md`, `software-technical-spec.md`) explicitly say `GOTO 1` with a space causes a parser error and that the C-style operator should be tightly written. Two possibilities: (a) these community examples were transcribed loosely and don't reflect what the original authors actually ran; (b) the no-space rule is overstated. **Production macros in `example-macros/`** consistently use the tight form `GOTO1` with no space. When copying patterns from this file, prefer the tight-spacing form.
